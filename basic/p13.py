...
'''
# 더하기 함수(2개의 정수값을 받아서 더한 값을 리턴하는 
# 함수 sum 구현)
def sum(x,y):
    return x + y
# 함수 호출 => 사용 
...
# 가능하게 mix()를 확장한 mixEx 구성하시오 => 딕셔너리
def mixEx(*args):
    addSum = 0
    addMul = 1
    for arg in args:
...
# 일종의 어플리케이션 전체 기능을 통제할 수 있는 환경번수들을
...
##############################################################
# 사용 타이밍
# 인터프리터 언어이기 때문에 먼저 정의된 후에 사용 가능하다.
# test()
def test():
    print('test')
##############################################################
...
def check():
    ...
    # a를 로컬변수가 아닌 전역변수를 가르키게 된다.
check()
print(a)
##############################################################
# 함수도 수행문이 하나면 옆으로 쓸수 있다.
