# ./templates: html 파일들이 위치하는곳
# ./static   : 리소스 파일들이 위치하는곳(js,css,img, ...)
# static 밑에 있는 모든 리소스는
# http://localhost/static/리소스명 이렇게 직접 접근 가능
# 즉, 라우팅 할 필요가 없다. 무조건 웹 경로로 인식한다.
'''
1. 이미 작성된 템플릿을 입히기
2. 템플릿 쪼개서 조합하기
3. 파일 업로드
...
# 실습할때는 서버 메모리에 들고 있을 것이고 서비스 할 때는
# 디비나, 서드 파트 솔루션을 이용하여 처리한다.
from flask import Flask, render_template, url_for, request, redirect, session, jsonify
from db.sql import *
import os

app = Flask(__name__)
app.secret_key = 'ghjkl;sdafjkfjasdkfjdask'
...
def uploadPhoto():
    if request.method =='GET':
        ...
        for f in request.files.getlist('fileData'):
            ...
            # 윈도우든 리눅스든 경로가 알아서 적용되게 구현해야 한다.
            # 어디다가 저장할 것인가? => 정적 url을 제공하는 위치
            # ~/static/upload
            # 사용자 별로 구분 => 아이디를 폴더로 or 파일명에 아이디를 붙이기
            # 동일 파일 구분 => 시간을 추가
            # 궁극적인 해결방안 => 중복되지 않는 해시값(32바이트)으로 이름변경
            path = os.path.join(os.getcwd(),'flask_advance','static','upload',f.filename)
            # 디스크상 저장 위치
            # C:\Users\student\Desktop\py_project\flask_advance\static\upload\Chrysanthemum.jpg
            # 웹 경로상
            # http://127.0.0.1:5000/static/upload/Chrysanthemum.jpg
        ...
        'content': '2', 
        'author': 'm', 
        'files': 'Chrysanthemum.jpg|Desert.jpg|Hydrangeas.jpg|Jellyfish.jpg|Koala.jpg|Lighthouse.jpg|Penguins.jpg|Tulips.jpg'}
        ...
        if result:
            ...
            return render_template('error.html', msg='등록성공',
                                url=url_for('uploadPhoto'))
            
        else:
            # 3-2 실패면 실패되었습니다. -> 돌아가기
            return render_template('error.html', msg='등록실패')
        # return "파일 업로드 처리 %s" % result
...
# 코드별 상세페이지
@app.route('/tradeList/<code>')
def info(code):
    '''
        # 코드값이 없으면 백 => 동적 파라미터 특성상 값이 없으면 404에러
        # if not code:
        #     return render_template('err.html',msg='정보누락')
        ##############################################################
    ...
    return render_template('sub/info.html',trade = selectOneStockInfo(code),
...
def updateStock():
    # 1. 파라미터 획득
    # ImmutableMultiDict([('cur', '1'), ('rate', '10')])
    # print(request.form)
    # 2. 쿼리 수행
    # 3. 수행 결과 판단
